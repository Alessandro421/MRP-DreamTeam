...
// ─── MRP ENGINE ──────────────────────────────────────────────────────────────
function buildSKU(raw) {
  const cat = CATEGORY_DEFAULTS[raw.category];
  return {
    ...raw,
...
function calcMRP(sku, horizon = 90) {
  const pos = OPEN_POS[sku.id] || [];
  let projectedStock = sku.stock;
  const curve = [];
  const orders = [];

  for (let d = 0; d <= horizon; d++) {
    // receipts from open POs
    const receipts = pos.filter(p => p.eta === d).reduce((s, p) => s + p.qty, 0);
    // receipts from planned orders
    const plannedReceipts = orders.filter(o => o.receiptDay === d).reduce((s, o) => s + o.qty, 0);
    projectedStock += receipts + plannedReceipts;
    projectedStock -= sku.avgDaily;

    curve.push({ day: d, stock: Math.round(projectedStock), safety: sku.safetyStock });

    // reorder trigger: if projected stock at end of lead time will fall below safety
    if (d % sku.reviewCycle === 0) {
      const futureDay = d + sku.leadTime;
      const stockAtFuture = projectedStock - sku.avgDaily * sku.leadTime +
      ...
      if (stockAtFuture < sku.safetyStock && futureDay <= horizon) {
        ...
        const raw = Math.max(need, sku.moq);
        const qty = Math.ceil(raw / sku.orderMultiple) * sku.orderMultiple;
        orders.push({ orderDay: d, receiptDay: futureDay, qty });
      }
    }
  }

  const daysOfStock = projectedStock / sku.avgDaily;
  const status = projectedStock < 0 ? "critical"
    : projectedStock < sku.safetyStock ? "at-risk"
    : daysOfStock < sku.leadTime / 7 ? "watch"
    : "ok";

  return { curve, orders, finalStock: projectedStock, daysOfStock, status };
}

// ─── COLOURS ─────────────────────────────────────────────────────────────────
const STATUS_COLOR = { critical: "#ef4444", "at-risk": "#f97316", watch: "#eab308", ok: "#22c55e" };
const STATUS_BG    = { critical: "#450a0a", "at-risk": "#431407", watch: "#422006", ok: "#052e16" };
const STATUS_LABEL = { critical: "CRITICAL", "at-risk": "AT RISK", watch: "WATCH", ok: "OK" };

// ─── MAIN APP ─────────────────────────────────────────────────────────────────
export default function MRPPlanner() {
  const [view, setView] = useState("workbench"); // workbench | exceptions | categories | detail
  ...
  const [skuOverrides, setSkuOverrides] = useState(
    Object.fromEntries(SKUS_RAW.map(s => [s.id, s.overrides]))
  );

  const skus = useMemo(() =>
    SKUS_RAW.map(raw => buildSKU({ ...raw, overrides: skuOverrides[raw.id] || {} })),
  [skuOverrides]);

  const mrpResults = useMemo(() =>
    Object.fromEntries(skus.map(s => [s.id, calcMRP(s)])),
  [skus]);
  ...
  const exceptions = useMemo(() =>
    enriched.filter(s => s.mrp.status === "critical" || s.mrp.status === "at-risk")
  ...
  const openDetail = (sku) => { setSelectedSKU(sku); setView("detail"); };
    ...
    <div style={{ fontFamily: "'IBM Plex Mono', 'Courier New', monospace", background: "#f0f2f5", minHeight: "100vh", color: "#1e293b" }}>
      ...
      <div style={{ borderBottom: "1px solid #e2e8f0", padding: "0 24px", display: "flex", alignItems: "center", gap: 24, height: 52, background: "#ffffff", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
        ...
        {["workbench", "exceptions", "categories"].map(v => (
          <button key={v} className={`nav-btn${view === v || (view === "detail" && v === "workbench") ? " active" : ""}`}
            style={{ color: view === v ? "#2563eb" : "#64748b" }}
            onClick={() => setView(v)}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <span className="tag">SKUs: {SKUS_RAW.length}</span>
      ...
      <div style={{ padding: 24 }} className="anim-in">
        ...
        {(view === "workbench" || view === "detail") && view !== "detail" && (
          <WorkbenchView enriched={filtered} filterCat={filterCat} setFilterCat={setFilterCat}
            ...
            onOpenDetail={openDetail} onEditParams={(sku) => setEditingParams(sku)}
            catParams={catParams} />
        )}
        ...
        {view === "detail" && selectedSKU && (
          <DetailView sku={selectedSKU} mrp={mrpResults[selectedSKU.id]}
            openPOs={OPEN_POS[selectedSKU.id] || []}
            onBack={() => setView("workbench")}
            onEditParams={() => setEditingParams(selectedSKU)} />
        )}

        {/* ── EXCEPTIONS ── */}
        {view === "exceptions" && (
          <ExceptionView exceptions={exceptions} onOpenDetail={openDetail} />
        )}

        {/* ── CATEGORIES ── */}
        {view === "categories" && (
          <CategoryView catSummary={catSummary} catParams={catParams}
            setCatParams={setCatParams} enriched={enriched} />
        )}
      </div>
      ...
      {editingParams && (
        <ParamModal sku={editingParams} catParams={catParams}
          ...
          onSave={(overrides) => {
            setSkuOverrides(prev => ({ ...prev, [editingParams.id]: overrides }));
...
// ─── WORKBENCH VIEW ───────────────────────────────────────────────────────────
function WorkbenchView({ enriched, filterCat, setFilterCat, filterStatus, setFilterStatus, search, setSearch, onOpenDetail, onEditParams }) {
  return (
    <div>
      <div style={{ display: "flex", gap: 10, marginBottom: 16, alignItems: "center", flexWrap: "wrap" }}>
        <input className="input" placeholder="Search SKU / name…" value={search}
      ...
      <div className="card" style={{ overflow: "auto" }}>
        <table>
          <thead>
            <tr>
              {["SKU", "Description", "Category", "Stock", "Safety Stock", "Days Cover", "Lead Time", "MOQ", "Open Orders", "Planned", "Status", ""].map(h => (
                <th key={h}>{h}</th>
              ))}
            </tr>
          </thead>
                ...
                <tr key={s.id} className="sku-row" onClick={() => onOpenDetail(s)}
                  ...
                  <td>
                    <button className="btn" style={{ padding: "3px 8px", fontSize: 10 }}
                      onClick={e => { e.stopPropagation(); onEditParams(s); }}>
                      PARAMS
                    </button>
                  </td>
                </tr>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 20 }}>
        {[
          ...
          <div key={k.label} className="stat-card">
            <div style={{ fontSize: 20, fontWeight: 600, color: k.highlight ? "#dc2626" : "#0f172a", fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div className="card" style={{ padding: 16 }}>
            ...
            {openPOs.length === 0 ? (
              <div style={{ color: "#334155", fontSize: 12 }}>No open POs</div>
            ) : openPOs.map((po, i) => (
          ...
          <div className="card" style={{ padding: 16, flex: 1 }}>
            ...
            {mrp.orders.length === 0 ? (
              <div style={{ color: "#334155", fontSize: 12 }}>No planned orders</div>
          ...
          <div className="card" style={{ padding: 16 }}>
            ...
            {[
              ["Lead Time", sku.leadTime + "d"],
              ["MOQ", sku.moq],
              ["Order Multiple", sku.orderMultiple],
              ["Review Cycle", sku.reviewCycle + "d"],
            ].map(([l, v]) => (
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 12 }}>
        {exceptions.map(s => {
          const status = s.mrp.status;
          return (
            <div key={s.id} className="card" style={{ padding: 18, borderColor: STATUS_COLOR[status] + "44", cursor: "pointer" }}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                {[
                  ...
                  { l: "Days Cover", v: s.mrp.daysOfStock > 0 ? Math.round(s.mrp.daysOfStock) + "d" : "OUT", warn: true },
                ].map(k => (
                  <div key={k.l} style={{ background: "#f8fafc", border: "1px solid #e2e8f0", padding: "8px 10px", borderRadius: 3 }}>
                    <div style={{ fontSize: 16, fontWeight: 600, color: k.warn ? STATUS_COLOR[status] : "#0f172a", fontFamily: "'IBM Plex Sans', sans-serif" }}>{k.v}</div>
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
        <div className="card" style={{ padding: 20 }}>
          <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 16 }}>
            SKU Status by Category
          </div>
        ...
        <div className="card" style={{ padding: 20 }}>
          <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 12 }}>
            Planning Logic Summary
          </div>
          <div style={{ fontSize: 11, color: "#64748b", lineHeight: 1.8 }}>
            <p style={{ marginBottom: 8 }}><span style={{ color: "#2563eb" }}>Reorder Point</span> = Safety Stock + (Avg Daily × Lead Time)</p>
            <p style={{ marginBottom: 8 }}><span style={{ color: "#2563eb" }}>Order Quantity</span> = CEIL(Net Req / Order Multiple) × Order Multiple, min MOQ</p>
            ...
            <p style={{ marginBottom: 8 }}><span style={{ color: "#ca8a04" }}>WATCH</span> = Days cover &lt; Lead Time (weeks)</p>
            <p><span style={{ color: "#16a34a" }}>OK</span> = Within all thresholds</p>
          </div>
        </div>
      </div>
          ...
          <tbody>
            {catSummary.map(row => (
              <tr key={row.cat}>
                ...
                <td>
                  <button className="btn" style={{ padding: "3px 8px", fontSize: 10 }}
                    onClick={() => { setEditCat(row.cat); setDraft({ ...catParams[row.cat] }); }}>
                    EDIT
                  </button>
                </td>
              </tr>
        ...
        <div className="modal-overlay" onClick={() => setEditCat(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#0f172a", marginBottom: 20 }}>
              Edit Category Defaults — {editCat}
            </div>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 20 }}>
              ...
              <button className="btn btn-primary" onClick={() => {
                setCatParams(prev => ({ ...prev, [editCat]: draft }));
                setEditCat(null);
              }}>Save</button>
            </div>
          </div>
      ...
      <div className="modal" onClick={e => e.stopPropagation()} style={{ minWidth: 460 }}>
        ...
        {fields.map(f => (
          <div key={f.key} style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
            ...
            <input className="input" type="number" placeholder={`Cat: ${f.catVal}`}
              value={draft[f.key]} style={{ width: 100, textAlign: "right" }}
              onChange={e => setDraft(d => ({ ...d, [f.key]: e.target.value }))} />
            {draft[f.key] !== "" && (
              <span style={{ fontSize: 10, color: "#2563eb" }}>OVERRIDE</span>
            )}
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 20 }}>
          ...
          <button className="btn btn-primary" onClick={() => {
            const overrides = {};
            Object.entries(draft).forEach(([k, v]) => { if (v !== "") overrides[k] = +v; });
            onSave(overrides);
